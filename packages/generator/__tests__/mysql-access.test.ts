...
import { generateAccessLayer } from "../src/targets/mysql/access.js";
...
describe("MySQL generateAccessLayer", () => {
  ...
  it("should use backtick quoting in SQL", () => {
    const schema = makeSchema();
  ...
  it("should import Pool from mysql2/promise", () => {
    const schema = makeSchema();
    ...
    expect(content).toContain('import type { Pool } from "mysql2/promise"');
    expect(content).toContain("pool: Pool");
    expect(content).not.toContain("sql: Sql");
  });

  it("should use pool.execute with parameter array", () => {
    const schema = makeSchema();
    ...
    expect(content).toContain("pool.execute(SQL_GET_ORDERS_BY_CUSTOMER, [params.customerId])");
  });
  ...
  it("should generate JOIN with backtick quoting for cross-entity shape fields", () => {
    const schema = makeSchema();
  ...
  it("should generate async function with Pool parameter", () => {
    const schema = makeSchema();
    ...
    expect(content).toContain("pool: Pool");
    expect(content).toContain("import type { Pool }");
  });
});
