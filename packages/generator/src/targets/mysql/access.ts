...
function generateSQLConstant(
  ...
  // WHERE clause — MySQL uses ? positional placeholders
...
function generateQueryFunction(
  ...
  // Build ordered parameter bindings for mysql2 execute()
  ...
  // Function body — mysql2 prepared statement
  if (bindings.length > 0) {
    const bindingList = bindings.join(", ");
    lines.push(
      `  const [rows] = await pool.execute(${sqlConst}, [${bindingList}]);`,
    );
  } else {
    lines.push(
      `  const [rows] = await pool.execute(${sqlConst});`,
    );
  }
...
function generateQueryFile(
  ...
  // Import mysql2/promise Pool type
  parts.push('import type { Pool } from "mysql2/promise";\n');
