...
function generateColumnDef(
  ...
  const sqliteType = fieldKindToSQLiteType(field.kind, field.role);
  const parts = [`  "${columnName}" ${sqliteType}`];
  ...
  if (isRefField(field)) {
    ...
    const cascade = cascadeRuleToSQLite(field.cascade);
...
function generatePIIComments(
  ...
  for (const [fieldName, field] of Object.entries(entity.fields)) {
    if (field.policy.pii) {
      const columnName = toSnakeCase(fieldName);
      comments.push(
        `-- PII: ${tableName}.${columnName} contains personally identifiable information`,
      );
    }
  }
...
function generateCreateTable(entity: EntityDefinition): string {
  ...
  const piiComments = generatePIIComments(entity, tableName);

  const lines: string[] = [];

  if (piiComments.length > 0) {
    lines.push(...piiComments);
  }
